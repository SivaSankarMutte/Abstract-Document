...
from App1.models import Songs,Movie
from App1.forms import SongsForm,MoviesForm,UsgForm,Rltype,Rlupd,Pfupd,Chgepwd
from App1.models import Rolereq,User
#EMAIL concept
from GreenWynk import settings
...
def moviesongslist(request,mid):
	...
	obj1=SendFav(request,uid=request.user.id)
	favlist=obj1.getfav()
...
def nextsong(request,sid,mid):
	...
	recmovie=obj1.getrecmovie()
	rdata=obj1.getRecData()
	obj2=SendFav(request,uid=request.user.id)
	favlist=obj2.getfav()
...
def updatesong(request,id):
	...
	if request.method=="POST":
		...
		if e.is_valid():
			e.save()
			messages.success(request,"{} Song updated Successfully".format(s.songname))
			return redirect('/addsong')
		else:
			print("Song Not updated")
			messages.success(request,"{} Song not updated".format(s.songname))
			return redirect('/addsong')

	e=SongsForm(instance=s)
	return render(request,'ht1/songupdate.html',{"q":e,'s':s})

def deletesong(request,id):
	s=Songs.objects.get(id=id)
	messages.success(request,"{} Song deleted successfully".format(s.songname))
	s.delete()
	return redirect('/addsong')
...
def favourites(request):
	...
	# 		mov.append(Movie.objects.get(id=song.mv_id))
	# print(slist)
	# print("Favsongs",favsongs)
	# data={}
	# for i in range(len(favsongs)):
	# 	data[i]=favsongs[i].songname,favsongs[i].song.url,mov[i].album.url,favsongs[i].id,mov[i].id
	# print(data)
	# print(data.values())
...
def srch(request):
	if request.method=="POST":
		s=request.POST['srch']
		mdata=Movie.objects.filter(moviename=s)
		if list(mdata)!=[]:
			return moviesongslist(request,mdata[0].id)
		data=Songs.objects.filter(songname=s)
		if list(data)!=[]:
			return playsong(request,data[0].id,data[0].mv_id)
		else:
			return render(request,'ht1/SongNotFound.html')
	return redirect(request.META['HTTP_REFERER'])
